...
(:mod:`agent_input_sanitizer.invisible`). There is deliberately no local copy and
no fallback: if the shared dependency is absent, :func:`default_charset` raises
(fail closed) rather than silently under-matching with a partial set.

TODO(published-dep): agent-input-sanitizer is not yet published as a versioned
distribution the redactor can pin. Until it is, the dependency is declared in
``pyproject.toml`` as a source/URL dependency and imported here; it must never be
replaced by an inlined copy of the code points (that fork is the exact silent
regression this indirection prevents).
"""
...
from agent_input_sanitizer.invisible import invisible_charset as _shared_charset
